...
namespace urban_archive.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(string returnUrl)
        {
            ...
            //{
            //string COOKIE_NAME_FOR_USER = "UserName";
            //HttpCookie cookie = System.Web.HttpContext.Current.Request.Cookies.Get(COOKIE_NAME_FOR_USER);

            //if (cookie != null)
            //{
            //    string UserName = cookie.Value;
            //    ViewBag.Name = new SelectList(db.AspNetUsers, "UserName", "UserName", UserName);
            //    ViewBag.ReturnUrl = returnUrl;
            //    //cookie.Values.Remove(COOKIE_NAME_FOR_USER);
            //    //System.Web.HttpContext.Current.Response.Cookies.Remove(COOKIE_NAME_FOR_USER);
            //    return View();
            //}
            //else
            //{
            ...
            ViewBag.ReturnUrl = returnUrl;
            var userlist = db_user.AspNetUsers.OrderBy(x => x.DepartmentId).ThenBy(x => x.RoleId);
            ViewBag.Name = new SelectList(userlist, "UserName", "UserName");
            return View();
        }
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            String  user_name = model.Name;
            int DepartmentID = db_user.AspNetUsers.Where(a=>a.UserName==user_name).FirstOrDefault().DepartmentId;
            //int RoleID = db_user.AspNetUsers.Where(a => a.UserName == user_name).FirstOrDefault().RoleId;

            // 这不会计入到为执行帐户锁定而统计的登录失败次数中
            // 若要在多次输入错误密码的情况下触发帐户锁定，请更改为 shouldLockout: true
            //model.RememberMe =bool.Parse(Request.Form["RememberMe"]);
            var result = await SignInManager.PasswordSignInAsync(model.Name, model.Password, model.RememberMe, shouldLockout: true);
            ViewBag.Name = new SelectList(db.AspNetUsers, "UserName", "UserName");
            ...
            List<SelectListItem> list = new List<SelectListItem>
                {
                    ...
                    new SelectListItem { Text = "信息管理科", Value = "1"},
                    ...
                    new SelectListItem { Text = "财务室", Value = "9" }
                 };
        ...
        public async Task<ActionResult> Register(RegisterViewModel model, int? DepartmentID, int? RoleID)
        {
            List<SelectListItem> list = new List<SelectListItem>
                {
                    new SelectListItem { Text = "请选择科室", Value = "0"},
            ...
            if (ModelState.IsValid)
            {
                ...
                var user = new ApplicationUser { UserName = model.Name, DepartmentName = model.DepartmentName, Email = model.Email, Password = model.Password, RoleName = model.Role, RoleId = model.RoleID, DepartmentId = model.DepartmentID };
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var user = new Models.ApplicationUser { UserName = model.Name, Email = model.Email };
