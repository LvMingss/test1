...
namespace SoftPOS
{
    public class PrinterHelper
    {
        ...
        #region API声明
        [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Auto)]//调整内存布局
        internal struct structPrinterDefaults//访问权限仅限于当前项目
        {
            [MarshalAs(UnmanagedType.LPTStr)]//指示如何在托管代码和非托管代码之间封送数据。简单点说，托管代码是一microsoft的中间语言，他主要的作用是在.NET FRAMEWORK的CLR执行代码前去编译源代码，也就是说托管代码充当着翻译的作用，源代码在运行时分为两个阶段：
             //1.源代码编译为托管代码；（所以源代码可以有很多种，如VB,C#,J#)
            //2.托管代码编译为microsoft系统的.net平台专用文件(如类库、可执行文件等)。
            public String pDatatype;
            public IntPtr pDevMode;
            [MarshalAs(UnmanagedType.I4)]	//4 字节有符号整数。
        ...
        public static List<String> GetPrinterList()
        {
            ...
            //}
            //return alRet;
            var defaultPrinterName = GetDeaultPrinterName();
            List<string> fPrinters = new List<string>();
            fPrinters.Add(defaultPrinterName);  //默认打印机出现在列表的第一项
